...
- **Event sourcing (TXT-first / CSV / XML-fallback)**: scored events are read via a shared `load_events()`.
  ...
  **Tool 10** copies tool 6's chain verbatim (TXT → CSV → XML, DataFrame with real onsets); its copy of
  ...
  - **Event-onset sidecar (`{file_id}_event_onsets.tsv`, tool 6 → tool 7)**: alongside the per-epoch event
    ...
    event_remap)`. Tool 7 otherwise only has per-epoch booleans and could not place a marker *inside* an
    epoch; with this file its per-epoch montage draws each onset as a labelled vertical line **without
    reloading the raw recording** (`load_event_onsets`, non-fatal when absent). Counts **all** mapped events
    (not just the flagging-selected types), like `build_event_counts`. **Optional/additive**: no events → no
    file, every other output byte-identical. It is a **montage companion**, so — like `{file_id}_context-epo.fif`
...
- **Rejection-method colour palette (single source, CVD-validated)**: the colour code for the six flagging
  ...
  **verbatim** in tool 6's `plot_rejection_heatmap` and in tools 8/8-voila (whose array is shorter and in a
  different order — it has no `event` — so it is remapped per method, never copy-pasted). 7bis imports the
  lib, so it follows automatically. The six method hues were selected by running a colour-blind separation
  check (OKLab ΔE under simulated protan/deutan) over candidate sets: hand-made "intuitive" sets that pair
  red with orange or green with gold collapse to ΔE 1.5–3.3, whereas the retained set reaches ≈7.2 — the
  best achievable for six simultaneous categories. Index 0 (`none`) stays **dark** so heatmap marks pop, and
  `multiple` is **cyan** (clearly outside the six method hues; it used to be a dark red confusable with
  `amplitude`). Six categories cannot reach the normal-vision ΔE floor of 15, so identity is **never carried
  by colour alone**: every plot ships a legend and/or method-coloured channel labels. `CTX_COLOR`
  (EOG/EMG/ECG context traces) deliberately sits **outside** the method hues (teal / olive / sienna) so a
  context trace never impersonates a rejection method. Changing the palette changes rendered PNGs (not
  byte-neutral) — re-run the Curry generators afterwards.
- **Fixed clinical display scales for per-epoch montages (`DISPLAY_SCALE_UV`)**: the tool-7 per-epoch
  montage allocates a **fixed** µV peak-to-peak range per channel row, **per channel type** —
  `{EEG 150, EOG 300, EMG 100, ECG 1000}` µV — instead of auto-scaling to the displayed window. Auto-scaling
  made the same waveform change height from one epoch to the next, which destroys visual amplitude criteria;
  with fixed scales a 75 µV slow wave always draws at half an EEG row. Values follow clinical PSG display
  conventions (AASM ~7 µV/mm for EEG/EOG on a ~2 cm row, high-gain chin EMG, 1 mV for ECG). Each type also
...
→ `specparam_artefact_detection.m`) uses the **same** Welch config (4 s window, 50 % overlap) but fits FOOOF
on the **smoothed** spectrum (`SmoothPower`), not the raw one — the single differentiator on the 1/f fit.
*Exact oscip order*: `SmoothPower = smooth_spectrum_median(EpochPower, freqs, MedianSmoothSpan=3)` then
`smooth_spectrum(SmoothPower, freqs, MeanSmoothSpan=2)`, so **median 3 Hz first, LOWESS 2 Hz second**.
*Implementation*: two helpers, applied in that order per (epoch, channel) on the **linear** power (matching
oscip; specparam logs it afterwards), each Hz span converted to a point count `span_pts =
round(span_hz/freq_res)`:
- `smooth_psd_median(psds_uV2, freqs, span_hz=3.0)` — `pandas.Series(...).rolling(window=span_pts,
  center=True, min_periods=1).median()` reproduces MATLAB `movmedian` (centred, truncated-window edges); it
  removes narrow spikes (residual line-noise harmonics, single-bin artefacts) that the LOWESS local-linear
  fit alone doesn't fully suppress. Uses **pandas** (already imported — no new dependency).
- `smooth_psd_lowess(psds_uV2, freqs, span_hz=2.0)` — `statsmodels …smoothers_lowess.lowess(y, freqs,
  frac=span_pts/n_freqs, it=0)` (~8 pts at 0.25 Hz), `it=0` = non-robust (MATLAB `'lowess'`, **not**
  `'rlowess'`).
...
- `{file_id}_all-epo.fif` — all epochs with `epochs.metadata` DataFrame (columns: `epoch_idx`, `stage`, `reject_flag`, `reject_method`, `flag_amplitude`, `flag_flat`, `flag_gradient`, `flag_1f_error`, `flag_1f_r2`, plus `flag_event` **when event flagging ran**, and one `evt_<canonical>` bool per canonical event type **when event flagging ran** — a per-epoch flag naming *which* event type flagged the epoch, since `flag_event` is only the OR; read by **tool 7**'s per-epoch inspector to show the culprit event(s)). The `flag_<method>` columns are **per-epoch "any channel" booleans** (the naive epoch-level decision, read by tool 7); the full per-(epoch, channel) mask **is** now persisted separately, in `{file_id}_epoch_channel_rejection.tsv` (below). The per-epoch/per-stage TSVs and `global_rejection_by_stage.tsv` gain the matching `flag_event` / `event` entries the same way — additively, so event-free runs stay byte-compatible with earlier outputs.
...
- `{file_id}_event_epoch_flags.tsv` — **optional, additive** per-epoch event-type flag table, written **only when event flagging ran on a file that has events**. One row per epoch, columns `file_id`, `epoch_idx`, `stage`, then one `evt_<canonical>` 0/1 per canonical event type selected for flagging (from the in-memory `event_type_masks`). Persists which event **type** flagged which epoch — the `_epoch_channel_rejection.tsv` only keeps the OR'd `flag_event` — so **tool 7bis** can re-select a subset of event types without reloading the raw events. Built by `build_event_epoch_flags()`. Absent → tool 7bis falls back to the OR'd `flag_event` (no sub-selection).
- `{file_id}_event_counts.tsv` — **optional, additive** per-canonical-type raw event count for the file (columns `event_type`, `n_events`, `n_epochs_flagged`), written under the same condition. `n_events` counts **all** scored events of that category in the recording (independent of which types were selected for flagging), so a downstream threshold can key on any category; `n_epochs_flagged` (from the mask) is reference-only (NaN for a type present in the file but not selected). Built by `build_event_counts()`. Drives **tool 7bis**'s per-event participant-exclusion threshold. These sidecars are **format-agnostic** (`event_type_masks`/`events_df` are format-neutral) → they pass through the Curry generator unchanged; re-run `_make_tool6_curry.py` after editing the event section.
- `{file_id}_event_onsets.tsv` — **optional, additive** per-event onset table (columns `type`, `onset_s`, `duration_s`; one row per scored event that maps to a canonical type, counting **all** mapped events like `_event_counts.tsv`), written under the same condition. Built by `build_event_onsets(events_df, event_remap)`. Lets **tool 7** draw event-onset markers *inside* an epoch on its per-epoch montage — the per-epoch flags alone cannot place a marker in time — **without reloading the raw recording**. Unlike the two sidecars above it is a **montage companion**, so it is written **beside the `.fif` in the raw-epochs dir** (`derivatives/raw_epo/<subtree>/`, like `{file_id}_context-epo.fif`), *not* under `reports_preprocessing/`: tool 7 loads companions from the fif's folder and never reads the reports tree. Also added to the stale-copy cleanup of the old flat `derivatives/<subtree>/` layout. Read on demand by `qc_rejected_epochs_lib.load_event_onsets` (non-fatal when absent → montage simply has no event markers).
...
- **Reports folder** (`fc_reports`) — holds `{file_id}_epoch_channel_rejection.tsv` (e.g.
  ...
  (cached at scan time as `S['tsv_by_fid']`, reused by the run). The optional tool-6 event sidecars
  (`{file_id}_event_epoch_flags.tsv`, `{file_id}_event_counts.tsv`) are also located here and cached
  (`S['flags_by_fid']` / `S['counts_by_fid']`).
- **Scan** lists the participants, warns (orange) when some have no matching TSV in the reports folder
  ...
  any custom stage — no `custom_stages.json` needed), populating one checkbox per stage. It also populates
  the **flagging-method checkboxes** (from the `flag_<method>` columns present, union across files) and the
  **event-type rows** (from the sidecars' `evt_<type>` columns + `_event_counts.tsv`).

**Section 2 — methods, thresholds & stages** (populated at Scan): besides the two threshold fields and the
stage checkboxes, one **checkbox per flagging method** (`amplitude/gradient/flat/1f_r2/1f_error/event`, all
ON by default = the previous `reject_any` behaviour) lets the user drop a method after reviewing the tool-6
reports. Ticking the **`event`** master reveals nested **event-type rows** (checkbox-revealed-widgets rule:
initial `display` derived from the checkbox value): one row per canonical type = `[✓] <type>` + an
...
deferred). The (epoch × channel) matrix fed to `auto_reject_decision` is **recomposed by
`build_pair_matrix(methods, event_master, event_types, event_flags_path)`** from the ticked methods rather
than read straight from `reject_any`: it ORs the selected `flag_<method>` columns, and for the `event`
method ORs the ticked `evt_<type>` columns of `{file_id}_event_epoch_flags.tsv` (epoch-level, broadcast
across channels like `flag_event`). With all methods + all event types on, the union equals the stored
`reject_any`, so the **default run is byte-identical** to the previous tool. If the event sidecar is absent
(pre-feature data), it **falls back** to the OR'd `flag_event` (all event types, no sub-selection) + a warning.

**Per-event participant exclusion** (before the write): for each ticked event type with a positive
threshold, `read_event_counts()` reads `{file_id}_event_counts.tsv`; if the file's raw `n_events` for that
type **exceeds** the threshold the **whole participant is excluded** — no clean-epo, a decision row + report
marked `excluded=True` with the reason (e.g. `312 arousal events > 200`), counted in a new `excluded` tally.

**Provenance**: the decision TSV / global summary gain `methods_used`, `event_types_used`, `excluded`,
`exclude_reason`; the clean-epo `metadata` gains `auto_reject_methods` / `auto_reject_event_types`.

**Edge cases** (non-fatal, add to `failed`, no clean-epo written): none of the selected stages present; no
method ticked; **all channels** would be dropped; **all in-scope epochs** rejected. (On sparse 3-channel
montages with high flagging the 20 % channel threshold can drop every channel — expected; the thresholds are
editable.) An **excluded** participant (event threshold) likewise gets a row + report but no clean-epo.
...
- **DATA** — `<raw_root.parent>/clean_epo_auto/<subtree>/{file_id}_clean-epo.fif`: `clean_epo_auto/` beside
  ...
  **only** file here is the `.fif` (selected-stage kept epochs, dropped channels removed; `metadata` gains
  `auto_reject_channel_pct` / `auto_reject_epoch_pct` / `auto_reject_stages` / `auto_reject_methods` /
  `auto_reject_event_types` provenance).
- **REPORTS** — `<reports_root.parent>/reports_rejection_auto/<subtree>/`: `reports_rejection_auto/` beside
  ...
  - `{file_id}_autoreject_decision.tsv` — one-row durable record **and** the global-summary row source
    (thresholds, stages, `methods_used`, `event_types_used`, `excluded`, `exclude_reason`,
    `rejected_channels`, per-channel `channel_badness_pct`, `n_epochs` /
...
`n_epochs_rejected` each with the % in parentheses, `n_epochs`, plus an `excluded` column with the reason
when any participant was excluded) + a `% epochs rejected per participant` bar
and the **full** table in `autoreject_database_report.html`, and `autoreject_failed.tsv`. The end-of-run
line tallies `saved / all-rejected / excluded / skipped / failed`. **Skip +
